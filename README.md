...
```bash
cmake -S . -B build-linux -DCNA_BACKEND_SDL_RENDERER=ON
...
```powershell
cmake -S . -B build-windows -DCNA_BACKEND_SDL_RENDERER=ON
...
Pick one with `-DCNA_GRAPHICS_BACKEND=<name>`. This vendored copy of `cna` keeps only three
backends (see `plan_lite.md` Phase 3 — every other backend `cna` upstream supports was removed
along with its source, CMake wiring, and docs):

- **`SDL_RENDERER`** (default, all platforms) — the real rendering path for the game.
- **`HEADLESS`** — no GPU/window at all, for driving the game's own logic under CI without a
  display server.
- **`SOFTWARE`** — a CPU rasterizer, also display-server-free.

Note: mobile-eggbert's `CNA_GRAPHICS_SOURCE_DIR` points at the vendored `cna/` copy inside this
repo (originally taken from `cna`'s `develop` branch).
