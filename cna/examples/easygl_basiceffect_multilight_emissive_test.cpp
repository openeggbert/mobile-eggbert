...
// shader-facing "emissive" uniform; Lighting.fxh's ComputeLights(eyeVector, worldNormal, numLights)
// sums up to 3 directional lights' `max(dot(-Direction,N),0)*DiffuseColor` contributions, then
// `result.Diffuse = sum * DiffuseColor.rgb + EmissiveColor`. CNA folds AmbientLightColor into the
// same light-sum multiply (mathematically equivalent net result — see BasicEffect::
// FillGpuDrawParams()'s comment) rather than pre-baking it into the emissive uniform, so the net
// formula this test checks is: **expected = (AmbientLightColor + sum_i NdotL_i*LightDiffuse_i for
// enabled lights) * DiffuseColor + EmissiveColor*Alpha**, texture kept white to isolate the formula.
//
// Before this task, BasicEffect::FillGpuDrawParams() only ever forwarded DirectionalLight0's
// direction/diffuse to the GPU, and never forwarded EmissiveColor at all on the lit path (only on
// the disabled-lighting path, Task 369) -- DirectionalLight1/DirectionalLight2 were silently
// dropped regardless of Enabled/DiffuseColor, and EmissiveColor was silently dropped whenever
// LightingEnabled=true.
//
// 3 checks, all using the same lit normal (NdotL=0.5 against direction (0,0,1), matching Task
// 368's precedent) and non-uniform per-channel light/emissive colors so each channel's origin is
// individually distinguishable:
//   1. All 3 lights enabled + nonzero EmissiveColor: proves light1/light2 are summed AND
//      EmissiveColor is added (both previously silently dropped).
//   2. DirectionalLight2.Enabled=false (DiffuseColor left nonzero): proves the Enabled-gating
//      already applied to light0 (Task 368) also applies to light2 -- its contribution must be
//      exactly zero, not just "some" value.
//   3. DirectionalLight1's direction rotated 90 degrees off-axis (NdotL1=0 for this test's
//      normal), light0/light2 back to their check-1 direction/enabled state: proves light1 reads
//      its OWN direction field rather than aliasing light0's (a copy-paste hazard given all 3
//      lights share near-identical FillGpuDrawParams code).
//
