...
// `bgfx::setTexture()` when a real texture was present, with no fallback at all — meaning a
// null-texture draw silently retained whatever texture the *previous* draw call had bound. Fixed
// by adding a `defaultWhiteTexture3D_` and an `else` branch to all 7 of Bgfx's texture-binding
// call sites (a general fix, not limited to `AlphaTestEffect`), matching EasyGL/Vulkan's own
...
class BgfxAlphaTestNullTextureTest : public Game
{
...
int main()
{
    BgfxAlphaTestNullTextureTest game;
    game.Run();
