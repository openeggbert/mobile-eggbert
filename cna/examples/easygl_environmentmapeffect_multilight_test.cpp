...
// worldNormal, numLights), summing up to 3 directional lights' `max(dot(-Direction,N),0)*
// DiffuseColor` contributions before multiplying by DiffuseColor.rgb and adding EmissiveColor —
// identical formula to BasicEffect's (Task 885), just without a per-light specular term (FNA's
// EnvironmentMapEffect.fx hardcodes every DirLightNSpecularColor to float3(0,0,0), since its own
// "specular" concept is the environment-map reflection amount, unrelated to per-light Blinn-Phong).
//
// Before this task, EnvironmentMapEffect::FillGpuDrawParams() only ever forwarded
// DirectionalLight0's direction/diffuse — DirectionalLight1/DirectionalLight2 were silently
// dropped regardless of Enabled/DiffuseColor.
//
// EnvironmentMapAmount=0 and EnvironmentMapSpecular=0 isolate the lit-diffuse formula from the
// cube-map blend entirely (same isolation trick as Task 393's amount-zero test) so this test only
// exercises the light-sum, not the env-map lerp.
//
// Same per-channel-color discrimination trick as Task 885's BasicEffect sibling test: each light's
// DiffuseColor only touches one colour channel, so channel drops are unambiguous:
//   1. All 3 lights enabled, shared direction (NdotL=0.5 each): every channel should show its own
//      light's contribution.
//   2. DirectionalLight2.Enabled=false: blue channel's contribution must drop to 0.
//   3. DirectionalLight1 rotated 90 degrees off-axis (NdotL1=0): green channel's contribution must
//      drop to 0, proving light1 reads its own Direction field rather than aliasing light0's.
//
...
class EnvironmentMapEffectMultiLightTest : public Game
{
...
public:
    EnvironmentMapEffectMultiLightTest()
    {
...
int main()
{
    EnvironmentMapEffectMultiLightTest game;
    game.Run();
