    ...
    NeoSdk::Property<std::string> Worlds::CurrentGameFilename{ [this]() { return "CurrentGame"; }};
      ...
      std::string Worlds::GetWorldFilename(int gamer, int rank) {
         ...
         bool Worlds::ReadGameData(byte data[], size_t dataSize)
        {
            ...
            std::ifstream file(GameDataFilename, std::ios::binary);
            if (!file.is_open()) {
                string gdf = GameDataFilename;
      ...
      void Worlds::WriteGameData(byte data[], size_t dataSize)
        ...
        std::ofstream file(GameDataFilename, std::ios::binary | std::ios::trunc);
        if (!file.is_open()) {
            string gdf = GameDataFilename;
      ...
      void Worlds::DeleteCurrentGame()
        ...
        const std::filesystem::path path{CurrentGameFilename};
        try {
      ...
      string Worlds::ReadCurrentGame()
        ...
        std::ifstream file(CurrentGameFilename, std::ios::binary);
        if (!file.is_open()) {
            string cgf = CurrentGameFilename;
      ...
      void Worlds::WriteCurrentGame(string& data)
        ...
        std::ofstream file(CurrentGameFilename, std::ios::out | std::ios::binary);
        if (file.is_open()) {
         ...
         void Worlds::GetIntArrayField(const string lines[], int lineCount, const string& section, int rank, const string& name, int array[], int& arraySize) {
            arraySize = 0;
         ...
         bool Worlds::GetBoolField(const string lines[], int lineCount, const string& section, int rank, const string& name) {
        ...
        return GetTypedField<int>(lines, lineCount, section, rank, name);
         ...
         double Worlds::GetDoubleField(const string lines[], int lineCount, const string& section, int rank, const string& name) {
     ...
     TinyPoint Worlds::GetPointField(const string lines[], int lineCount, const string& section, int rank, const string& name) {
        for (int i = 0; i < lineCount; i++) {
            const string& text = lines[i];
            if (!text.starts_with(section + ":") || rank-- != 0) {
                continue;
     ...
     void Worlds::GetDoorsField(const string lines[], int lineCount, const string& section, int doors[], int& doorCount) {
        doorCount = 0;
        for (int i = 0; i < lineCount; i++) {
            const string& text = lines[i];
